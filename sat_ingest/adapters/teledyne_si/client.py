...
class TeledyneSiAdapter:
    ...
    def search(self, params)->Iterable[SearchResult]:
        colls=params.collections or ["desis-hyperspectral"]
        t=int(time.time())
        return [SearchResult(id=f"{c}-{t}-0", collection=c, datetime=str(params.time or "1970-01-01")) for c in colls]
    ...
    def download(self, item:SearchResult, asset_keys:Optional[List[str]]=None,
        ...
        keys=asset_keys or [f"band_{i:03d}" for i in range(1, 6)]
