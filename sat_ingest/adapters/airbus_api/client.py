...
class AirbusApiAdapter:
    ...
    def search(self, params)->Iterable[SearchResult]:
        collections = params.collections or ["terrasar-x-spotlight","spot-various"]
        t=int(time.time())
        return [SearchResult(id=f"{c}-{t}-{i}", collection=c, datetime=str(params.time or "1970-01-01"))
                for i,c in enumerate(collections[:max(1, params.limit or 1)])]

    def _write_gray_png(self, p:Path, size=(64,64)):
        if Image is None or np is None:
            p.write_bytes(b"PNG PLACEHOLDER"); return
    ...
    def download(self, item:SearchResult, asset_keys:Optional[List[str]]=None,
        ...
        keys=asset_keys or ["HH","HV","VH","VV"]
        out:Dict[str,DownloadedAsset]={}
        for k in keys:
            f=d/f"{k}.png"; self._write_gray_png(f); out[k]=DownloadedAsset(k,str(f))
        return out
